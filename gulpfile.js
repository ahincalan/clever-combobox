...
gulp.task('backup.component.tmp', function () {
  return gulp.src('./src/clever-combobox.component.ts').pipe(gulp.dest('./tmp'));
});

gulp.task('minify.css', function () {
  return gulp.src('./src/clever-combobox.component.css')
    .pipe(cleancss({compatibility: 'ie8'}))
    .pipe(gulp.dest('./tmp'));
});

gulp.task('minify.html', function () {
  return gulp.src('./src/clever-combobox.component.html')
    .pipe(htmlmin({collapseWhitespace: true, caseSensitive: true}))
    .pipe(gulp.dest('./tmp'));
});

gulp.task('inline.template.and.styles.to.component', function () {
  var styles = fs.readFileSync('./tmp/clever-combobox.component.css', 'utf-8');
  var htmlTpl = fs.readFileSync('./tmp/clever-combobox.component.html', 'utf-8');

  styles = styles.split('\\e').join('\\\\e');

  return gulp.src(['./src/clever-combobox.component.ts'])
...
gulp.task('delete.modified.component', function () {
  return gulp.src(['./src/clever-combobox.component.ts'], {read: false}).pipe(clean());
});

gulp.task('restore.original.component', function () {
  return gulp.src('./tmp/clever-combobox.component.ts').pipe(gulp.dest('./src/'));
});
...
gulp.task('tslint', function () {
  return gulp.src([
    "src/clever-combobox.pipe.ts",
    "src/clever-combobox.interface.ts",
    "src/index.ts",
    "src/clever-combobox.component.ts",
    "src/clever-combobox.module.ts"])
    .pipe(tslint({
